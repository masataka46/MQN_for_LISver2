...
class CnnDqnAgent(object):
    ...
    time_M = 11 #now and 10history used for MQN
    ...
    def agent_start(self, observation):
        ...
        # Initialize State
        # modify state size for MQN
        self.state = np.zeros((self.q_net.hist_size * self.time_M, self.q_net_input_dim), dtype=np.uint8)
        self.state[0] = obs_array
        state_ = np.asanyarray(self.state.reshape(1, self.q_net.hist_size * self.time_M, self.q_net_input_dim), dtype=np.float32)
        if self.use_gpu >= 0:
    ...
    def agent_step(self, reward, observation):
        ...
        if self.q_net.hist_size == 4:
            ...
            for i in range(self.time_M - 1):
                self.state[self.time_M - i - 1] = self.state[self.time_M - i -2]
            #print 'now in agent_step finished for bun'
            self.state[0] = obs_array

            #print 'in agent_step self.state.shape=',
            #print self.state.shape
            #self.state = np.asanyarray([obs_array], dtype=np.uint8)
            self.state_tmp = np.asanyarray([obs_array], dtype=np.uint8)
        else:
    ...
    def agent_step_update(self, reward, action, eps, q_now, obs_array):
        ...
        if self.policy_frozen is False:  # Learning ON/OFF
            self.q_net.stock_experience(self.time, self.last_state, self.last_action, reward, self.state_tmp, False)
    ...
    def agent_end(self, reward):  # Episode Terminated
        ...
        # Learning Phase
        if self.policy_frozen is False:  # Learning ON/OFF
            self.q_net.stock_experience(self.time, self.last_state, self.last_action, reward, self.state_tmp,
                                        True)
            self.q_net.experience_replay(self.time)
