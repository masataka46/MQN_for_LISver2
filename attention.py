...
#import retrievalFunc
import attentionFunc

class Attention(link.Link):
    ...
    def __init__(self, left_size, right_size, out_size, nobias=True,
                 initialW=None, initial_bias=None):
        super(Attention, self).__init__(W=(left_size, right_size, out_size))
    ...
    def __call__(self, e1, e2):
        ...
        """
        if self.nobias:
            return attentionFunc.attentionFunc(e1, e2, self.W)
        else:
            return attentionFunc.attentionFunc(e1, e2, self.W, self.V1, self.V2, self.b)
